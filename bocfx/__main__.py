...
def main(FX, sort, time, plot, csv, pt, op, bar):
    ...
    if plot != 0:
        ...
        p = np.array(output)
        fl = len(FX_or)
        plt.figure(figsize=(3.6, 1.9))  # [修改] 修改图像大小以适配窗口
        plt.grid()  # [修改] 显示网格

        for f in range(fl):
            ax = plt.subplot(fl,1,f+1)
            ax.xaxis.set_major_formatter(mdate.DateFormatter('%H'))  # [修改] 只显示小时数
            ...
            # plt.legend(loc=2)  # [修改] 不显示线的说明标签

        op = os.path.abspath('.') + "/images"   # [修改] 修改图标缓存文件的路径
        # op = os.path.expanduser(op)
        if not os.path.exists(op):
            os.makedirs(op)
        filename = "cache_price"
        plotpath = os.path.join(op, filename)+'.png'
        
        plt.savefig(plotpath, dpi=150, bbox_inches='tight')  # [修改] 增加 bbox_inches='tight' 防止 x 轴被截断
